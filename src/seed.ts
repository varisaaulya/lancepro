...
export function seedData() {
  ...
  for (const u of users) {
    ...
    if (!exists) {
      ...
      db.prepare('INSERT INTO users (nama, email, password, profesi) VALUES (?, ?, ?, ?)').run(u.nama, u.email, hash, u.profesi);
    }
  }
  ...
  if (clientCount.count === 0) {
    const admin = db.prepare('SELECT id FROM users WHERE email = ?').get('admin@lancepro.com') as { id: number };
    if (admin) {
      const clients = [
      ...
      for (const [perusahaan, nama, email, telp, alamat] of clients) {
        db.prepare('INSERT INTO clients (user_id, nama, perusahaan, email, telepon, alamat) VALUES (?, ?, ?, ?, ?, ?)')
          .run(admin.id, nama, perusahaan, email, telp, alamat);
      }

      // Add Projects
      const clientIds = db.prepare('SELECT id FROM clients').all() as { id: number }[];
      const projects = [
      ...
      for (let i = 0; i < projects.length; i++) {
        ...
        db.prepare('INSERT INTO projects (user_id, client_id, nama, kategori, nilai_kontrak, status, prioritas, progress, tanggal_mulai, deadline) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
          .run(admin.id, clientId, nama, kat, nilai, status, prio, prog, '2024-01-01', '2024-06-01');
      }

      // Add Invoices
      const projectIds = db.prepare('SELECT id FROM projects').all() as { id: number }[];
      for (let i = 0; i < 4; i++) {
        const project = db.prepare('SELECT client_id, nama, nilai_kontrak FROM projects WHERE id = ?').get(projectIds[i].id) as { client_id: number, nama: string, nilai_kontrak: number };
        const total = project.nilai_kontrak;
        const status = i % 2 === 0 ? 'Lunas' : 'Belum Dibayar';
        const res = db.prepare('INSERT INTO invoices (user_id, client_id, project_id, nomor_invoice, total, status, tanggal_jatuh_tempo) VALUES (?, ?, ?, ?, ?, ?, ?)')
          .run(admin.id, project.client_id, projectIds[i].id, `INV-2024-00${i+1}`, total, status, '2024-12-31');
        
        db.prepare('INSERT INTO invoice_items (invoice_id, nama_item, qty, harga_satuan, subtotal) VALUES (?, ?, ?, ?, ?)')
      ...
      for (let i = 0; i < 8; i++) {
        db.prepare('INSERT INTO gallery (user_id, judul, deskripsi, kategori, gambar, tahun) VALUES (?, ?, ?, ?, ?, ?)')
